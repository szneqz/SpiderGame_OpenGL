...
void Model::addTextures(const string& diffuse, const string& specular)
{
		if (exists(diffuse))
		{
			Texture diff;
			diff.id = loadImageToTexture(diffuse.c_str());
			diff.path = diffuse.substr(diffuse.find_last_of('/') + 1, diffuse.length());
			diff.type = "texture_diffuse";
			tempTextures.push_back(diff);
		}

		if (exists(specular))
		{
			Texture spec;
			spec.id = loadImageToTexture(specular.c_str());
			spec.path = specular.substr(specular.find_last_of('/') + 1, specular.length());
			spec.type = "texture_specular";
			tempTextures.push_back(spec);
		}
}

void Model::initShaders(unsigned int shader_program)
{
...
void Model::draw(unsigned int shaders_program)
{
...
void Model::loadModel(const string& path)
{
	...
	cout << "scene->mNumAnimations: " << scene->mNumAnimations << endl;
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	return Mesh(vertices, indices, tempTextures, bones_id_weights_for_each_vertex);
}

GLuint Model::loadImageToTexture(const char* image_path)
{
...
void  Model::setCurrentAnimation(unsigned int nr)
{
	if (nr == currentAnimation)
		return;

	if (nr > scene->mNumAnimations - 1)
	{
		std::cout << "ERROR: Wrong animation number, setting to 0!" << std::endl;
		currentAnimation = 0;
	}
	else
	{
	...
	//ustalam czestotliwosc aktualnej animacji
	if (scene->mAnimations[currentAnimation]->mTicksPerSecond != 0.0)
	{
		ticks_per_second = scene->mAnimations[currentAnimation]->mTicksPerSecond;
	}
	else
	{
		ticks_per_second = 25.0f;
...
void Model::readNodeHierarchy(float p_animation_time, const aiNode* p_node, const aiMatrix4x4 parent_transform)
{
	...
	if (node_anim)
	{
		...
		//Kod dotyczący obrotu daną częścią ciała PODCZAS animacji
...
void Model::boneTransform(double time_in_sec, vector<aiMatrix4x4>& transforms)
{
	...
	float animation_time = fmod(time_in_ticks, scene->mAnimations[currentAnimation]->mDuration);
